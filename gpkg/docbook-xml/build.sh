...
TERMUX_PKG_DEPENDS="libxml2-utils-glibc"
...
termux_step_make_install() {
	...
	$TERMUX_PREFIX/bin/xmlcatalog --noout --create "$TERMUX_PREFIX/etc/xml/docbook-xml"
	...
	for ver in 4.1.2 4.{2..5}; do
		...
		case $ver in
			4.1.2) xml=' XML' ;;&
			*)
				$TERMUX_PREFIX/bin/xmlcatalog --noout --add "public" \
			...
			;;&
			*)
				$TERMUX_PREFIX/bin/xmlcatalog --noout --add "rewriteSystem" \
					"http://www.oasis-open.org/docbook/xml/$ver" \
					"$TERMUX_PREFIX/share/xml/docbook/xml-dtd-$ver" \
					"$TERMUX_PREFIX/etc/xml/docbook-xml"
				$TERMUX_PREFIX/bin/xmlcatalog --noout --add "rewriteURI" \
					"http://www.oasis-open.org/docbook/xml/$ver" \
					"$TERMUX_PREFIX/share/xml/docbook/xml-dtd-$ver" \
					"$TERMUX_PREFIX/etc/xml/docbook-xml"
			;;&
...
termux_step_create_debscripts() {
	...
	if [ "$TERMUX_PACKAGE_FORMAT" = "pacman" ] || [ "\$1" = "configure" ]; then
		if [ ! -e "$TERMUX_PREFIX/etc/xml/catalog" ]; then
			LD_PRELOAD='' $TERMUX_PREFIX/bin/xmlcatalog --noout --create "$TERMUX_PREFIX/etc/xml/catalog"
		else
			LD_PRELOAD='' $TERMUX_PREFIX/bin/xmlcatalog --noout --del "file://$TERMUX_PREFIX/etc/xml/docbook-xml" \
				$TERMUX_PREFIX/etc/xml/catalog
		fi
		LD_PRELOAD='' $TERMUX_PREFIX/bin/xmlcatalog --noout --add "delegatePublic" \
		...
		LD_PRELOAD='' $TERMUX_PREFIX/bin/xmlcatalog --noout --add "delegatePublic" \
			...
			"file://$TERMUX_PREFIX/etc/xml/docbook-xml" \
			$TERMUX_PREFIX/etc/xml/catalog
		LD_PRELOAD='' $TERMUX_PREFIX/bin/xmlcatalog --noout --add "delegateSystem" \
			"http://www.oasis-open.org/docbook/" \
			"file://$TERMUX_PREFIX/etc/xml/docbook-xml" \
			$TERMUX_PREFIX/etc/xml/catalog
		LD_PRELOAD='' $TERMUX_PREFIX/bin/xmlcatalog --noout --add "delegateURI" \
			"http://www.oasis-open.org/docbook/" \
			"file://$TERMUX_PREFIX/etc/xml/docbook-xml" \
			$TERMUX_PREFIX/etc/xml/catalog
	fi
