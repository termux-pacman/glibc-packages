...
termux_step_create_debscripts() {
	...
	if [ "$TERMUX_PACKAGE_FORMAT" = "pacman" ] || [ "\$1" = "configure" ]; then
		if [ ! -e "$TERMUX_PREFIX/etc/xml/catalog" ]; then
			...
			LD_PRELOAD="" $TERMUX_PREFIX/bin/xmlcatalog --noout --del "$TERMUX_PREFIX/share/xml/docbook/xsl-stylesheets-$TERMUX_PKG_VERSION" \
				"$TERMUX_PREFIX/etc/xml/catalog"
		fi

		for ver in $TERMUX_PKG_VERSION current; do
			for x in rewriteSystem rewriteURI; do
				LD_PRELOAD="" $TERMUX_PREFIX/bin/xmlcatalog --noout --add \$x http://cdn.docbook.org/release/xsl/\$ver \
					"$TERMUX_PREFIX/share/xml/docbook/xsl-stylesheets-$TERMUX_PKG_VERSION" \
					"$TERMUX_PREFIX/etc/xml/catalog"

				LD_PRELOAD="" $TERMUX_PREFIX/bin/xmlcatalog --noout --add \$x http://docbook.sourceforge.net/release/xsl-ns/\$ver \
					"$TERMUX_PREFIX/share/xml/docbook/xsl-stylesheets-$TERMUX_PKG_VERSION" \
					"$TERMUX_PREFIX/etc/xml/catalog"

				LD_PRELOAD="" $TERMUX_PREFIX/bin/xmlcatalog --noout --add \$x http://docbook.sourceforge.net/release/xsl/\$ver \
