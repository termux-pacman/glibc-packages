...
termux_step_make_install() {
	...
	if [ "$TERMUX_ON_DEVICE_BUILD" = "true" ]; then
		...
		cp -r ${TERMUX_PKG_BUILDDIR}/libc.so ${glibc_dir}/${TERMUX_LIB_PATH}/libc.so.6
		LD_PRELOAD="" LD_LIBRARY_PATH="" /system/bin/cp -r ${glibc_dir}/${TERMUX_LIB_PATH}/* ${TERMUX_LIB_PATH}
	fi
	make install
...
termux_step_make_install32() {
	...
	local hpath
	for hpath in $(find ${glibc32_dir}/${TERMUX_PREFIX}/include -type f); do
		local h=$(sed "s|${glibc32_dir}/${TERMUX_PREFIX}/include/||g" <<< "$hpath")
		if [ -f "${TERMUX_PREFIX}/include/${h}" ] && \
			[ $(md5sum "${hpath}" | awk '{printf $1}') = $(md5sum "${TERMUX_PREFIX}/include/${h}" | awk '{printf $1}') ]; then
			rm "${hpath}"
		fi
	done
	find ${glibc32_dir}/${TERMUX_PREFIX}/include -type d -empty -delete
	echo "DONE"

	cp -r ${glibc32_dir}/${TERMUX_LIB_PATH} $TERMUX_PREFIX
	install -dm755 ${TERMUX_PREFIX}/include32
	cp -r ${glibc32_dir}/${TERMUX_PREFIX}/include/* ${TERMUX_PREFIX}/include32
