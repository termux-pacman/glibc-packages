...
case $ARCH in
	"aarch64")
		export GPKG_DEV_FLAGS+=" -Wl,--dynamic-linker=$GLIBC_PREFIX/lib/ld-linux-aarch64.so.1"
		export GPKG_DEV_CC="aarch64-linux-gnu-gcc"
		export GPKG_DEV_CARCH="aarch64";;
	"arm")
		export GPKG_DEV_FLAGS+=" -Wl,--dynamic-linker=$GLIBC_PREFIX/lib/ld-linux-armhf.so.3"
		export GPKG_DEV_CC="arm-linux-gnueabihf-gcc"
		export GPKG_DEV_CARCH="armv7h"
		export GPKG_DEV_TARGET="armv7l-linux-gnueabihf";;
	"x86_64")
		export GPKG_DEV_FLAGS+=" -Wl,--dynamic-linker=$GLIBC_PREFIX/lib/ld-linux-x86-64.so.2"
		export GPKG_DEV_CC="x86_64-linux-gnu-gcc"
		export GPKG_DEV_CARCH="x86_64";;
	"i686")
		export GPKG_DEV_FLAGS+=" -Wl,--dynamic-linker=$GLIBC_PREFIX/lib/ld-linux.so.2"
